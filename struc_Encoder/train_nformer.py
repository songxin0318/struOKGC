...
# from cole_code import KGCDataModule
# from cole_code import NFormer, Knowformer
...
class NFormerTrainer:
    ...
    def _load_dataset(self, config: dict):
        ...
        tokenizer = AutoTokenizer.from_pretrained('/home/h3c/data/xinSong/xs2/prompt-tuning/SapBERT-from-PubMedBERT-fulltext')

        # 2. resize tokenizer, load datasets
        data_module = KGCDataModule(config, tokenizer, encode_struc=True)  #在dataset.py文件中
        # KGCDataModule 数据集加载类(dataset.py),通过调用内部函数,返回想要的数值
    ...
    def _train_one_epoch(self):
        self.model.train()
        outputs = list()
        for batch_idx, batch_data in enumerate(self.train_dl):    #训练过程 batch_data的遍历
